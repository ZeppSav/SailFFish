...
inline void KER_RVM_DGC_FD2(const TensorGrid &gss, TensorGrid &g_out, const dim3 &id, const dim3 &D, const Real &H, const Real &Fac)
{
    // This kernel implements Step 1 (FD2) of the RVM calculation as described in the publication by Caprace et al. doi: 10.1007/s00162-019-00510-1
    // We calculate the term in paranethsis in Eq. (4)

    // Calculate curl terms
    int ids;
    Real doydx=0.,dozdx=0., doxdy=0., dozdy=0., doxdz=0., doydz=0.;
    ids = GID(id.x-1,id.y  ,id.z  ,D);                                      doydx += D1C2[0]*gss[1][ids];       dozdx += D1C2[0]*gss[2][ids];
    ids = GID(id.x+1,id.y  ,id.z  ,D);                                      doydx += D1C2[1]*gss[1][ids];       dozdx += D1C2[1]*gss[2][ids];
    ids = GID(id.x  ,id.y-1,id.z  ,D);     doxdy += D1C2[0]*gss[0][ids];                                        dozdy += D1C2[0]*gss[2][ids];
    ids = GID(id.x  ,id.y+1,id.z  ,D);     doxdy += D1C2[1]*gss[0][ids];                                        dozdy += D1C2[1]*gss[2][ids];
    ids = GID(id.x  ,id.y  ,id.z-1,D);     doxdz += D1C2[0]*gss[0][ids];    doydz += D1C2[0]*gss[1][ids];
    ids = GID(id.x  ,id.y  ,id.z+1,D);     doxdz += D1C2[1]*gss[0][ids];    doydz += D1C2[1]*gss[1][ids];

    Real InvH = 1.0/H;
    int tid = GID(id.x,id.y,id.z,D);
    g_out[0][tid] += Fac*(dozdy-doydz)*InvH;
    g_out[1][tid] += Fac*(doxdz-dozdx)*InvH;
    g_out[2][tid] += Fac*(doydx-doxdy)*InvH;
}

inline void KER_RVM_DGC_FD4(const TensorGrid &gss, TensorGrid &g_out, const dim3 &id, const dim3 &D, const Real &H, const Real &Fac)
{
    // This kernel implements Step 1 (FD2) of the RVM calculation as described in the publication by Caprace et al. doi: 10.1007/s00162-019-00510-1
    // We calculate the term in paranethsis in Eq. (4)
    ...
    int ids;
    Real doydx=0.,dozdx=0., doxdy=0., dozdy=0., doxdz=0., doydz=0.;
    ids = GID(id.x-2,id.y  ,id.z  ,D);                                  doydx += D1C4[0]*gss[1][ids];   dozdx += D1C4[0]*gss[2][ids];
    ids = GID(id.x-1,id.y  ,id.z  ,D);                                  doydx += D1C4[1]*gss[1][ids];   dozdx += D1C4[1]*gss[2][ids];
    ids = GID(id.x+1,id.y  ,id.z  ,D);                                  doydx += D1C4[2]*gss[1][ids];   dozdx += D1C4[2]*gss[2][ids];
    ids = GID(id.x+2,id.y  ,id.z  ,D);                                  doydx += D1C4[3]*gss[1][ids];   dozdx += D1C4[3]*gss[2][ids];
    ids = GID(id.x  ,id.y-2,id.z  ,D);  doxdy += D1C4[0]*gss[0][ids];                                   dozdy += D1C4[0]*gss[2][ids];
    ids = GID(id.x  ,id.y-1,id.z  ,D);  doxdy += D1C4[1]*gss[0][ids];                                   dozdy += D1C4[1]*gss[2][ids];
    ids = GID(id.x  ,id.y+1,id.z  ,D);  doxdy += D1C4[2]*gss[0][ids];                                   dozdy += D1C4[2]*gss[2][ids];
    ids = GID(id.x  ,id.y+2,id.z  ,D);  doxdy += D1C4[3]*gss[0][ids];                                   dozdy += D1C4[3]*gss[2][ids];
    ids = GID(id.x  ,id.y  ,id.z-2,D);  doxdz += D1C4[0]*gss[0][ids];   doydz += D1C4[0]*gss[1][ids];
    ids = GID(id.x  ,id.y  ,id.z-1,D);  doxdz += D1C4[1]*gss[0][ids];   doydz += D1C4[1]*gss[1][ids];
    ids = GID(id.x  ,id.y  ,id.z+1,D);  doxdz += D1C4[2]*gss[0][ids];   doydz += D1C4[2]*gss[1][ids];
    ids = GID(id.x  ,id.y  ,id.z+2,D);  doxdz += D1C4[3]*gss[0][ids];   doydz += D1C4[3]*gss[1][ids];
...
inline void KER_RVM_DGC_FD6(const TensorGrid &gss, TensorGrid &g_out, const dim3 &id, const dim3 &D, const Real &H, const Real &Fac)
{
    // This kernel implements Step 1 (FD2) of the RVM calculation as described in the publication by Caprace et al. doi: 10.1007/s00162-019-00510-1
    // We calculate the term in paranethsis in Eq. (4)
    ...
    int ids;
    Real doydx=0.,dozdx=0., doxdy=0., dozdy=0., doxdz=0., doydz=0.;
    ids = GID(id.x-3,id.y  ,id.z  ,D);                                      doydx += D1C6[0]*gss[1][ids];  dozdx += D1C6[0]*gss[2][ids];
    ids = GID(id.x-2,id.y  ,id.z  ,D);                                      doydx += D1C6[1]*gss[1][ids];  dozdx += D1C6[1]*gss[2][ids];
    ids = GID(id.x-1,id.y  ,id.z  ,D);                                      doydx += D1C6[2]*gss[1][ids];  dozdx += D1C6[2]*gss[2][ids];
    ids = GID(id.x+1,id.y  ,id.z  ,D);                                      doydx += D1C6[3]*gss[1][ids];  dozdx += D1C6[3]*gss[2][ids];
    ids = GID(id.x+2,id.y  ,id.z  ,D);                                      doydx += D1C6[4]*gss[1][ids];  dozdx += D1C6[4]*gss[2][ids];
    ids = GID(id.x+3,id.y  ,id.z  ,D);                                      doydx += D1C6[5]*gss[1][ids];  dozdx += D1C6[5]*gss[2][ids];
    ids = GID(id.x  ,id.y-3,id.z  ,D);     doxdy += D1C6[0]*gss[0][ids];                                   dozdy += D1C6[0]*gss[2][ids];
    ids = GID(id.x  ,id.y-2,id.z  ,D);     doxdy += D1C6[1]*gss[0][ids];                                   dozdy += D1C6[1]*gss[2][ids];
    ids = GID(id.x  ,id.y-1,id.z  ,D);     doxdy += D1C6[2]*gss[0][ids];                                   dozdy += D1C6[2]*gss[2][ids];
    ids = GID(id.x  ,id.y+1,id.z  ,D);     doxdy += D1C6[3]*gss[0][ids];                                   dozdy += D1C6[3]*gss[2][ids];
    ids = GID(id.x  ,id.y+2,id.z  ,D);     doxdy += D1C6[4]*gss[0][ids];                                   dozdy += D1C6[4]*gss[2][ids];
    ids = GID(id.x  ,id.y+3,id.z  ,D);     doxdy += D1C6[5]*gss[0][ids];                                   dozdy += D1C6[5]*gss[2][ids];
    ids = GID(id.x  ,id.y  ,id.z-3,D);     doxdz += D1C6[0]*gss[0][ids];   doydz += D1C6[0]*gss[1][ids];
    ids = GID(id.x  ,id.y  ,id.z-2,D);     doxdz += D1C6[1]*gss[0][ids];   doydz += D1C6[1]*gss[1][ids];
    ids = GID(id.x  ,id.y  ,id.z-1,D);     doxdz += D1C6[2]*gss[0][ids];   doydz += D1C6[2]*gss[1][ids];
    ids = GID(id.x  ,id.y  ,id.z+1,D);     doxdz += D1C6[3]*gss[0][ids];   doydz += D1C6[3]*gss[1][ids];
    ids = GID(id.x  ,id.y  ,id.z+2,D);     doxdz += D1C6[4]*gss[0][ids];   doydz += D1C6[4]*gss[1][ids];
    ids = GID(id.x  ,id.y  ,id.z+3,D);     doxdz += D1C6[5]*gss[0][ids];   doydz += D1C6[5]*gss[1][ids];
...
inline void KER_RVM_DGC_FD8(const TensorGrid &gss, TensorGrid &g_out, const dim3 &id, const dim3 &D, const Real &H, const Real &Fac)
{
    // This kernel implements Step 1 (FD2) of the RVM calculation as described in the publication by Caprace et al. doi: 10.1007/s00162-019-00510-1
    // We calculate the term in paranethsis in Eq. (4)
    ...
    int ids;
    Real doydx=0.,dozdx=0., doxdy=0., dozdy=0., doxdz=0., doydz=0.;
    ids = GID(id.x-4,id.y  ,id.z  ,D);                                  doydx += D1C8[0]*gss[1][ids];   dozdx += D1C8[0]*gss[2][ids];
    ids = GID(id.x-3,id.y  ,id.z  ,D);                                  doydx += D1C8[1]*gss[1][ids];   dozdx += D1C8[1]*gss[2][ids];
    ids = GID(id.x-2,id.y  ,id.z  ,D);                                  doydx += D1C8[2]*gss[1][ids];   dozdx += D1C8[2]*gss[2][ids];
    ids = GID(id.x-1,id.y  ,id.z  ,D);                                  doydx += D1C8[3]*gss[1][ids];   dozdx += D1C8[3]*gss[2][ids];
    ids = GID(id.x+1,id.y  ,id.z  ,D);                                  doydx += D1C8[4]*gss[1][ids];   dozdx += D1C8[4]*gss[2][ids];
    ids = GID(id.x+2,id.y  ,id.z  ,D);                                  doydx += D1C8[5]*gss[1][ids];   dozdx += D1C8[5]*gss[2][ids];
    ids = GID(id.x+3,id.y  ,id.z  ,D);                                  doydx += D1C8[6]*gss[1][ids];   dozdx += D1C8[6]*gss[2][ids];
    ids = GID(id.x+4,id.y  ,id.z  ,D);                                  doydx += D1C8[7]*gss[1][ids];   dozdx += D1C8[7]*gss[2][ids];
    ids = GID(id.x  ,id.y-4,id.z  ,D);  doxdy += D1C8[0]*gss[0][ids];                                   dozdy += D1C8[0]*gss[2][ids];
    ids = GID(id.x  ,id.y-3,id.z  ,D);  doxdy += D1C8[1]*gss[0][ids];                                   dozdy += D1C8[1]*gss[2][ids];
    ids = GID(id.x  ,id.y-2,id.z  ,D);  doxdy += D1C8[2]*gss[0][ids];                                   dozdy += D1C8[2]*gss[2][ids];
    ids = GID(id.x  ,id.y-1,id.z  ,D);  doxdy += D1C8[3]*gss[0][ids];                                   dozdy += D1C8[3]*gss[2][ids];
    ids = GID(id.x  ,id.y+1,id.z  ,D);  doxdy += D1C8[4]*gss[0][ids];                                   dozdy += D1C8[4]*gss[2][ids];
    ids = GID(id.x  ,id.y+2,id.z  ,D);  doxdy += D1C8[5]*gss[0][ids];                                   dozdy += D1C8[5]*gss[2][ids];
    ids = GID(id.x  ,id.y+3,id.z  ,D);  doxdy += D1C8[6]*gss[0][ids];                                   dozdy += D1C8[6]*gss[2][ids];
    ids = GID(id.x  ,id.y+4,id.z  ,D);  doxdy += D1C8[7]*gss[0][ids];                                   dozdy += D1C8[7]*gss[2][ids];
    ids = GID(id.x  ,id.y  ,id.z-4,D);  doxdz += D1C8[0]*gss[0][ids];   doydz += D1C8[0]*gss[1][ids];
    ids = GID(id.x  ,id.y  ,id.z-3,D);  doxdz += D1C8[1]*gss[0][ids];   doydz += D1C8[1]*gss[1][ids];
    ids = GID(id.x  ,id.y  ,id.z-2,D);  doxdz += D1C8[2]*gss[0][ids];   doydz += D1C8[2]*gss[1][ids];
    ids = GID(id.x  ,id.y  ,id.z-1,D);  doxdz += D1C8[3]*gss[0][ids];   doydz += D1C8[3]*gss[1][ids];
    ids = GID(id.x  ,id.y  ,id.z+1,D);  doxdz += D1C8[4]*gss[0][ids];   doydz += D1C8[4]*gss[1][ids];
    ids = GID(id.x  ,id.y  ,id.z+2,D);  doxdz += D1C8[5]*gss[0][ids];   doydz += D1C8[5]*gss[1][ids];
    ids = GID(id.x  ,id.y  ,id.z+3,D);  doxdz += D1C8[6]*gss[0][ids];   doydz += D1C8[6]*gss[1][ids];
    ids = GID(id.x  ,id.y  ,id.z+4,D);  doxdz += D1C8[7]*gss[0][ids];   doydz += D1C8[7]*gss[1][ids];
...
static const Real Sigma = 0.1;              // Coresize
static const Real Sigma2 = Sigma*Sigma;     // Coresize^2
static const Real r0 = 1.0;                 // Radius
//static const Real Rey = 400;                // Reynold number of ring
//static const Real Gamma = Rey*Kin_Visc;     // Circulation

inline void KER_Init_Ring_Vort(const Real &x, const Real &y, const Real &z, Real &Ox, Real &Oy, Real &Oz, const Real &Rey, const Real &Nu)
{
    // This initializes the vorticity to be that of a ring
    Real Gamma = Rey*Nu;                                    // Circulation
    Real r = sqrt(y*y + z*z);
    Real th = atan2(z,y);
    Real d2 = (r-r0)*(r-r0) + x*x;
    //    Real Fac = Gamma/(M_PI*Sigma2)*exp(-d2/Sigma2);       // Standard Vortex Ring

    Real t = Sigma2/(4.0*Nu);
    Real Fac = Gamma*FourPIinv/(Nu*t)*exp(-d2/Sigma2);     // Saffman style distribution

    // Now specify vorticity (take negative of this)
    Ox = 0.0;
    Oy = -Fac*sin(th);
    Oz =  Fac*cos(th);
}

inline void KER_Init_Blob(const Real &x, const Real &y, const Real &z, Real &Ox, Real &Oy, Real &Oz)
{
    // This initializes the vorticity to be that of a ring
    Real r = sqrt(x*x + y*y + z*z);
    Real r0 = 0.2;
    Real Fac = FourPIinv*exp(-r*r/(r0*r0));     // Saffman style distribution
...
inline void KER_Lap_Test(const Real &x, const Real &y, const Real &z, Real &Ox, Real &Oy, Real &Oz)
{
...
//--- Perturbed vortex ring.
//--- This generates the perturbed vortex ring for the breakdown / turbulence model tests as in Cocle

// static const Real R = 1.0;                      // Ring radius

// Cocle test 1
//static const Real a = 0.4131/R;
//static const Real Gamma0 = 5500*Kin_Visc;
// static const Real eps = 0.0002;

//-- Cocle test 2
//static const Real a = 0.2;
//static const Real Gamma0 = 25000*Kin_Visc;
//static const Real eps = 0.0001;

//-- Liska case
//static const Real Gamma0 = 7500*Kin_Visc;
//static const Real a = 0.2*R;

//-- Winckelmans colliding vortices case
//static const Real Gamma0 = 400*Kin_Visc;    // Winckelmans collision
//static const Real a = 0.2*R;

//--- Tests for Saffman ring velocity
//static const Real a = 0.2*R;
//static const Real Gamma0 = 100.0*Kin_Visc;

// Constants
//static const Real a2 = a*a;
//static const Real t0 = R*R/Gamma0;

inline void Generate_Random_Phases(RVector &PhaseShift)
{
    //--- Generate array of random phase angles
    int NModesCocle = 24;
    for (int i=0; i<NModesCocle; i++) PhaseShift[i] = (rand()*1.0)/RAND_MAX*M_2PI;
}
