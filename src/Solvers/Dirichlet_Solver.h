...
class Poisson_Dirichlet_1D : public Solver_1D_Scalar
{
protected:
...
public:
    ...
    //--- Constructor
    Poisson_Dirichlet_1D(Grid_Type G = STAGGERED, Bounded_Kernel B = PS);

    //--- Solver setup
    SFStatus FFT_Data_Setup()
    {
        if (Grid==REGULAR)          return Setup_1D(gNX-2);
...
class Poisson_Dirichlet_2D : public Solver_2D_Scalar
{
protected:
...
public:
    ...
    //--- Constructor
    Poisson_Dirichlet_2D(Grid_Type G = STAGGERED, Bounded_Kernel B = PS);

    //--- Solver setup
    SFStatus FFT_Data_Setup()
    {
        if (Grid==REGULAR)          return Setup_2D(gNX-2,gNY-2);
...
class Poisson_Dirichlet_3D : public Solver_3D_Scalar
{
protected:
...
public:
    ...
    //--- Constructor
    Poisson_Dirichlet_3D(Grid_Type G = STAGGERED, Bounded_Kernel B = PS);

    //--- Solver setup
    SFStatus FFT_Data_Setup()
    {
        if (Grid==REGULAR)          return Setup_3D(gNX-2,gNY-2,gNZ-2);
...
class Poisson_Dirichlet_3DV : public Solver_3D_Vector
{
protected:
...
public:
    ...
    //--- Constructor
    Poisson_Dirichlet_3DV(Grid_Type G = STAGGERED, Bounded_Kernel B = PS);
